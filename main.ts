...
async function handleJsonRequest(data: any) {
  try {
    ...
    const userInput = data.input; // 直接从 data 中获取 input
    const userFileURL = data.fileURL; // 直接从 data 中获取 fileURL
    ...
    if (Array.isArray(userFileURL)) {
      currentFileURLs = userFileURL.filter((url: any) => typeof url === "string");
    } else if (typeof userFileURL === "string" && userFileURL.includes(',')) {
      currentFileURLs = userFileURL.split(',').map((url: string) => url.trim()).filter(Boolean);
    } else if (typeof userFileURL === "string") {
    ...
    if (currentUserParts.length === 0 && finalContents.length === 0) {
      return new Response(JSON.stringify({
        success: false,
        response: "No valid content found in input/fileURL or MessageHistory to send to Gemini.",
        fileDatas: [] // 明确返回空数组
      }), {
        status: 400,
